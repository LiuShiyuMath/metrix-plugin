...
beat() {
  local n="$1" prompt="$2" max="${3:-220}" sentinel="__BEAT_${1}_DONE__"
  if [ "${DEMO_FAKE:-0}" = "1" ]; then
    # pipeline self-test: no model, instant deterministic output
    send "echo \"[beat $n] $(printf '%s' "$prompt" | cut -c1-60)...\" ; sleep 1 ; echo $sentinel"
    local w=0; while :; do sleep 1; w=$((w+1));
      tmux -S "$SOCK" capture-pane -p -t "$SESSION" 2>/dev/null | grep -q "$sentinel" && break
      [ "$w" -ge 20 ] && break; done
    sleep 1; return
  fi
  send "claudefast --plugin-dir '$PLUGIN' -p '$prompt' ; echo $sentinel"
  local waited=0
  while :; do
    sleep 3; waited=$((waited+3))
    tmux -S "$SOCK" capture-pane -p -t "$SESSION" 2>/dev/null \
      | grep -q "$sentinel" && break
    [ "$waited" -ge "$max" ] && break
  done
  sleep 2
}
...
sleep 1
send "echo \"issue: $ISSUE\"; echo \"state: \$TEAMAGENT_WORKFLOW_FILE\""
sleep 1

beat 1 "运行命令 /teamagent-workflow:workflow-enable on 开启强制流程，用一句话报告结果。" 170

beat 2 "${ISO}我要对 issue ${ISSUE} 直接做 Stage 4 proof（证据 https://example.com/p）。按 workflow-proof-comment 要求先跑 bin/workflow-gate.sh；若 allowed:false 就把 reason 一字不差打印并停止，绝不发 gh pr comment。" 220

beat 3 "${ISO}对 issue ${ISSUE} 执行 Stage 1 claim：先跑 bin/workflow-gate.sh '<issue>' claimed，仅当 allowed:true 才用 jq -nc 追加一行 {ts,issue_url,stage:claimed,actor} 到 \$TEAMAGENT_WORKFLOW_FILE，并打印门禁 reason。" 220

beat 4 "${ISO}对 issue ${ISSUE} 执行 Stage 2 grill：先跑 bin/workflow-gate.sh '<issue>' grilled，仅当 allowed:true 才追加 stage=grilled 行到 \$TEAMAGENT_WORKFLOW_FILE，打印门禁 reason。" 220

beat 5 "${ISO}对 issue ${ISSUE} 执行 Stage 3 handoff：先跑 bin/workflow-gate.sh '<issue>' handoff，仅当 allowed:true 才追加 stage=handoff 行到 \$TEAMAGENT_WORKFLOW_FILE，打印门禁 reason。" 220

beat 6 "${ISO}现在 issue ${ISSUE} 已到 handoff，执行 Stage 4 proof：跑 bin/workflow-gate.sh '<issue>' proof，这次应当 allowed:true，打印门禁 reason（顺序走完后这道门才放行）。" 220

send "echo; echo '== state file (append-only, what the gate reads) =='; cat \$TEAMAGENT_WORKFLOW_FILE | jq -c '{stage,issue_url}'"
sleep 4
send "echo '== demo complete: blocked the skip, allowed the ordered path =='"
sleep 3
